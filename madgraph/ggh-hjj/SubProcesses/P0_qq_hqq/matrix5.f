...
C     
      ...
      COMMON/ML5_0_4_5_JAMP2/ML_JAMP2

      REAL*8 ML_AMP2(NLOOPDIAGRAMS)
      COMMON/ML5_0_4_5_AMP2/ML_AMP2
...
C       We don't use the poles, so let's not compute them with COLLIER
        CALL ML5_0_4_5_COLLIER_COMPUTE_UV_POLES(.FALSE.)
        CALL ML5_0_4_5_COLLIER_COMPUTE_IR_POLES(.FALSE.)
...
C        several helicity for the same PS point
        IF ((.NOT.FORCE_ML_HELICITY_SUM).AND.ISUM_HEL.EQ.0) THEN
          CALL ML5_0_4_5_SET_AUTOMATIC_CACHE_CLEARING(.FALSE.)
        ENDIF
        CALL ML5_0_4_5_GET_ANSWER_DIMENSION(ML_ANS_DIMENSION)
...
C       Of course this option can also not be used for polarized beams.
      ...
      IF(FORCE_ML_HELICITY_SUM )THEN
        ...
        CALL ML5_0_4_5_SLOOPMATRIX_THRES(P,ANS_ML,-1.0D0,PREC_FOUND
     $   ,ML_RET_CODE)
        CALL PROCESS_MADLOOP_OUTPUT5(P,T_BEFORE,ANS,ANS_ML(1,0)
     ...
     $       .LE.MAXTRIES.OR.ISUM_HEL.NE.0) THEN
              ...
              CALL CPU_TIME(T_BEFORE)
              CALL ML5_0_4_5_SLOOPMATRIXHEL_THRES(P,I,ANS_ML,-1.0D0
     $         ,PREC_FOUND,ML_RET_CODE)
              CALL PROCESS_MADLOOP_OUTPUT5(P,T_BEFORE,T,ANS_ML(1,0)
     $         ,ANS_ML(2,0),ANS_ML(3,0),PREC_FOUND(0),ML_RET_CODE)
...
C             Set right sign for ANS, based on sign of chosen helicity
 ...
 10       CONTINUE
     ...
     $       /HEL_JACOBIAN))
            AMP2(SUBDIAG(5)) = AMP2(SUBDIAG(5)) + (INTERFERENCE_CONTRIB
     $       /NMULTICHANNELS)
...
C     This last line is a tag do not remove it. 
      ...
      SUBROUTINE PROCESS_MADLOOP_OUTPUT5(P,TIME_BEFORE, ME_ANSWER,
