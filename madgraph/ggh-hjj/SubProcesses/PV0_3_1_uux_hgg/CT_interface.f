...
C     ===========================================
      SUBROUTINE ML5_0_3_1_CTLOOP(NLOOPLINE,PL,M2L,RANK,RES,STABLE)
C     
...
C     
      EXTERNAL ML5_0_3_1_LOOPNUM
      EXTERNAL ML5_0_3_1_MPLOOPNUM
C     
C     GLOBAL VARIABLES
C     
      INCLUDE 'coupl.inc'
      ...
      COMMON/ML5_0_3_1_LOOP/ID,SQSOINDEX,R
...
C     CONVERT THE MOMENTA FLOWING IN THE LOOP LINES TO CT CONVENTIONS
      ...
      CALL CTSXCUT(CTMODE,LSCALE,MU_R,NLOOPLINE,ML5_0_3_1_LOOPNUM
     $ ,ML5_0_3_1_MPLOOPNUM,RANK,PCT,M2LCT,RES,ACC,R1,STABLE)
...
C     CALL CTSINIT(THRS,LOOPLIB)
      ...
      SUBROUTINE ML5_0_3_1_BUILD_KINEMATIC_MATRIX(NLOOPLINE,P_LOOP,M2L
     $ ,S_MAT)
C     
...
C            component
     ...
     $          + ABS(P_LOOP(J,K))
      ...
      SUBROUTINE ML5_0_3_1_MP_BUILD_KINEMATIC_MATRIX(NLOOPLINE,P_LOOP
     $ ,M2L,S_MAT)
C     
C     Helper function that compute the loop kinematic matrix with
C      proper thresholds
...
C     ===========================================
      ...
      SUBROUTINE ML5_0_3_1_NINJA_LOOP(NLOOPLINE,PL,M2L,RANK,RES,STABLE)
C     
C     Module used
C     
      USE MNINJA
...
C      the loop is read for CTMode equal to 2 or 4.
      CALL ML5_0_3_1_SWITCH_ORDER(CTMODE,NLOOPLINE,PL,PDEN_DUMMY,M2L)
...
C     coefficients must be changed accordingly
      CALL ML5_0_3_1_INVERT_MOMENTA_IN_POLYNOMIAL(CURR_MAXCOEF
     $ ,TENSORCOEFS)
...
C     Compute the kinematic matrix
      DO J=1,NLOOPLINE
        ...
        ENDDO
      ENDDO
      CALL ML5_0_3_1_BUILD_KINEMATIC_MATRIX(NLOOPLINE,P_S_MAT,M2L
     $ ,S_MAT)
...
C     
      SUBROUTINE ML5_0_3_1_MP_NINJA_LOOP(NLOOPLINE,PL,M2L,RANK,RES
     $ ,STABLE)
C     
C     Module used
C     
...
C     
     ...
     $ ,NINJAINIT,COLLIERINIT
      ...
      COMMON/ML5_0_3_1_MP_LCOEFS/MP_LOOPCOEFS
...
C     For the direction test, we must switch the direction in which
C      the loop is read for CTMode equal to 2 or 4.
      CALL ML5_0_3_1_MP_SWITCH_ORDER(CTMODE,NLOOPLINE,PL,PDEN_DUMMY
     $ ,MP_M2L)
...
C     The loop momentum is in fact q_loop -> -q_loop, so that the
C     coefficients must be changed accordingly
      CALL MP_ML5_0_3_1_INVERT_MOMENTA_IN_POLYNOMIAL(CURR_MAXCOEF
     $ ,MP_TENSORCOEFS)
...
C     Compute the kinematic matrix
      DO J=1,NLOOPLINE
        ...
        ENDDO
      ENDDO
      CALL ML5_0_3_1_MP_BUILD_KINEMATIC_MATRIX(NLOOPLINE,P_S_MAT
     $ ,MP_M2L,MP_S_MAT)
...
C     WRITE(*,*) 'QP Ninja:  Loop ID',ID,' =',RES(1),RES(2),RES(3)
      ...
      SUBROUTINE ML5_0_3_1_INITNINJA()
C     
C     Module used
C     
      USE MNINJA
...
C     3 -> QCDLOOP
     ...
     $   //' CTLoopLibrary in MadLoopParams.dat.'
      ...
      SUBROUTINE ML5_0_3_1_LOOP_4(W1, W2, W3, W4, M1, M2, M3, M4, 
...
C       Choose the correct loop library
        CALL ML5_0_3_1_CHOOSE_LOOPLIB(LIBINDEX,NLOOPLINE,RANK
     $   ,COMPLEX_MASS,ID,DOING_QP,I_LIB)
        IF(MLREDUCTIONLIB(I_LIB).EQ.1)THEN
C         CutTools is used
          CALL ML5_0_3_1_CTLOOP(NLOOPLINE,PL,M2L,RANK,AMPL(1,NCTAMPS
     $     +LOOPNUM),S(1,LOOPNUM))
        ELSEIF (MLREDUCTIONLIB(I_LIB).EQ.6) THEN
          IF (.NOT.DOING_QP) THEN
            CALL ML5_0_3_1_NINJA_LOOP(NLOOPLINE,PL,M2L,RANK,AMPL(1
     $       ,NCTAMPS+LOOPNUM),S(1,LOOPNUM))
          ELSE
            CALL ML5_0_3_1_MP_NINJA_LOOP(NLOOPLINE,MP_PL,M2L,RANK
     $       ,AMPL(1,NCTAMPS+LOOPNUM),S(1,LOOPNUM))
          ENDIF

        ELSE
C         Tensor Integral Reduction is used 
          CALL ML5_0_3_1_TIRLOOP(1,LOOPNUM,I_LIB,NLOOPLINE,PL,M2L,RANK
     $     ,AMPL(1,NCTAMPS+LOOPNUM),S(1,LOOPNUM))
        ENDIF
      ELSE
      ...
      SUBROUTINE ML5_0_3_1_LOOP_3(W1, W2, W3, M1, M2, M3,  RANK,
