...
C     
      ...
      COMMON/ML5_0_3_1_JAMP2/ML_JAMP2

      REAL*8 ML_AMP2(NLOOPDIAGRAMS)
      COMMON/ML5_0_3_1_AMP2/ML_AMP2
...
C       We don't use the poles, so let's not compute them with COLLIER
        CALL ML5_0_3_1_COLLIER_COMPUTE_UV_POLES(.FALSE.)
        CALL ML5_0_3_1_COLLIER_COMPUTE_IR_POLES(.FALSE.)
...
C        several helicity for the same PS point
        IF ((.NOT.FORCE_ML_HELICITY_SUM).AND.ISUM_HEL.EQ.0) THEN
          CALL ML5_0_3_1_SET_AUTOMATIC_CACHE_CLEARING(.FALSE.)
        ENDIF
        CALL ML5_0_3_1_GET_ANSWER_DIMENSION(ML_ANS_DIMENSION)
...
C       Of course this option can also not be used for polarized beams.
      ...
      IF(FORCE_ML_HELICITY_SUM )THEN
        ...
        CALL ML5_0_3_1_SLOOPMATRIX_THRES(P,ANS_ML,-1.0D0,PREC_FOUND
     $   ,ML_RET_CODE)
     ...
     $       .LE.MAXTRIES.OR.ISUM_HEL.NE.0) THEN
              ...
              CALL CPU_TIME(T_BEFORE)
              CALL ML5_0_3_1_SLOOPMATRIXHEL_THRES(P,I,ANS_ML,-1.0D0
