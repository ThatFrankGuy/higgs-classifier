...
C     --=========================================--
      ...
      SUBROUTINE ML5_0_2_2_SLOOPMATRIX(P_USER,ANS)
C     
...
C      provide QP
      ...
      COMMON/ML5_0_2_2_STABILITY_TESTS/MAXSTABILITYLENGTH
...
C      DIFFERENT EVALUATION METHODS IN ORDER TO ASSESS STABILITY.
     ...
     $ /NSQSOXNLG*.FALSE./
      ...
      COMMON/ML5_0_2_2_RUNTIME_OPTIONS/AUTOMATIC_CACHE_CLEARING
...
C      stability check to take place
      ...
      COMMON/ML5_0_2_2_BYPASS_CHECK/BYPASS_CHECK, ALWAYS_TEST_STABILITY
C     
C     FUNCTIONS
C     
      INTEGER ML5_0_2_2_TIRCACHE_INDEX
      INTEGER ML5_0_2_2_ML5SOINDEX_FOR_BORN_AMP
      INTEGER ML5_0_2_2_ML5SOINDEX_FOR_LOOP_AMP
      INTEGER ML5_0_2_2_ML5SQSOINDEX
      INTEGER ML5_0_2_2_ISSAME
      LOGICAL ML5_0_2_2_ISZERO
      LOGICAL ML5_0_2_2_IS_HEL_SELECTED
...
C     
      ...
      COMMON/ML5_0_2_2_DOUBLECHECK_JAMP/RES_FROM_JAMP
     $ ,DIRECT_ME_COMPUTATION,ME_COMPUTATION_FROM_JAMP
      ...
      DATA N_DP_EVAL/1/
      DATA N_QP_EVAL/1/
      COMMON/ML5_0_2_2_N_EVALS/N_DP_EVAL,N_QP_EVAL
...
C      AVAILABLE OR NOT
     ...
     $ ,.TRUE.,.TRUE./
      COMMON/ML5_0_2_2_LOOPLIBS_AV/ LOOPLIBS_AVAILABLE
...
C      FOR STABILITY PURPOSE
      ...
      REAL*8 LSCALE
      INTEGER CTMODE
      COMMON/ML5_0_2_2_CT/LSCALE,CTMODE
      LOGICAL MP_PS_SET
...
C     Typically, the hel_offset is 10000
      ...
      INTEGER HELPICKED
      DATA HELPICKED/-1/
      COMMON/ML5_0_2_2_HELCHOICE/HELPICKED
      INTEGER USERHEL
      DATA USERHEL/-1/
      COMMON/ML5_0_2_2_USERCHOICE/USERHEL
...
C      subroutine of MadLoopCommons.dat
      ...
      COMMON/ML5_0_2_2_LOOP_TOOLS/QP_TOOLS_AVAILABLE,INDEX_QP_TOOLS

      COMPLEX*16 W(20,NWAVEFUNCS)
      COMMON/ML5_0_2_2_W/W

      COMPLEX*32 MPW(20,NWAVEFUNCS)
      COMMON/ML5_0_2_2_MP_W/MPW
...
C      coefficients when changing the CTMode for the stability test.
      ...
      COMMON/ML5_0_2_2_SKIP_COEFS/SKIP_LOOPNUM_COEFS_CONSTRUCTION

      LOGICAL TIR_DONE(NLOOPGROUPS)
      COMMON/ML5_0_2_2_TIRCACHING/TIR_DONE
      ...
      COMMON/ML5_0_2_2_LOOPRES/LOOPRES,S

      INTEGER CF_D(NCOLORROWS,NLOOPAMPS)
      INTEGER CF_N(NCOLORROWS,NLOOPAMPS)
      COMMON/ML5_0_2_2_CF/CF_D,CF_N

      INTEGER HELC(NEXTERNAL,NCOMB)
      COMMON/ML5_0_2_2_HELCONFIGS/HELC

      REAL*8 PREC,USER_STAB_PREC
      DATA USER_STAB_PREC/-1.0D0/
      COMMON/ML5_0_2_2_USER_STAB_PREC/USER_STAB_PREC
...
C     RETURNCODE=100*RET_CODE_H+10*RET_CODE_T+RET_CODE_U
      ...
      COMMON/ML5_0_2_2_ACC/ACCURACY,RET_CODE_H,RET_CODE_T,RET_CODE_U

      LOGICAL MP_DONE_ONCE
      DATA MP_DONE_ONCE/.FALSE./
      COMMON/ML5_0_2_2_MP_DONE_ONCE/MP_DONE_ONCE
...
C      reading the parameters
     ...
     $ ,FORCED_CHOICE_OF_COLLIER_IR_POLE_COMPUTATION/.FALSE.,.FALSE./
      COMMON/ML5_0_2_2_COLLIERPOLESFORCEDCHOICE/FORCED_CHOICE_OF_COLLIE
...
C      independently by each SubProcess.
      ...
      COMMON/ML5_0_2_2_FPE_IN_REDUCTION/FPE_IN_DP_REDUCTION,
     $  FPE_IN_QP_REDUCTION
...
C      requirement.
      ...
      COMMON/ML5_0_2_2_BEAM_POL/POLARIZATIONS
...
C       Setup the file paths
        ...
        CALL ML5_0_2_2_SET_N_EVALS(N_DP_EVAL,N_QP_EVAL)
...
C         We write a dummy filter for structural reasons here
 ...
 104    CONTINUE
     ...
     $   //' ML5_0_2_2_ColorNumFactors.dat. File not found'
 105    CONTINUE
        CLOSE(1)
 ...
 106    CONTINUE
        STOP 'Color factors could not be initialized from file'
     $   //' ML5_0_2_2_ColorDenomFactors.dat. File not found'
 107    CONTINUE
        CLOSE(1)
 ...
 108    CONTINUE
     ...
     $   //' from file ML5_0_2_2_HelConfigs.dat. File not found'
 109    CONTINUE
        CLOSE(1)
...
C     This code is therefore placed before 205 and after 200.
      CALL ML5_0_2_2_REINITIALIZE_CUMULATIVE_ARRAYS()
...
C      precision or when TIR_CACHE_SIZE<2.
     ...
     $ .NE.5).AND.(ML5_0_2_2_TIRCACHE_INDEX(CTMODE).EQ.(TIR_CACHE_SIZE
     $ +1))) THEN
        CALL ML5_0_2_2_CLEAR_TIR_CACHE()
      ENDIF
...
C      to the computation of the next helicity.
 205  CONTINUE
      ...
      IF (.NOT.MP_PS_SET.AND.(CTMODE.EQ.0.OR.CTMODE.GE.4)) THEN
        CALL ML5_0_2_2_SET_MP_PS(P_USER)
        MP_PS_SET = .TRUE.
      ENDIF
...
C     Check if we directly go to multiple precision
      IF (CTMODE.GE.4) THEN
        CALL ML5_0_2_2_MP_COMPUTE_LOOP_COEFS(MP_P,BUFFR_BIS)
        IF ((.NOT.DIRECT_ME_COMPUTATION).AND.ME_COMPUTATION_FROM_JAMP)
     $    THEN
...
C         Handle the possible requirement of specific polarizations
     ...
     $0,0).EQ.0.AND.(.NOT.ML5_0_2_2_IS_HEL_SELECTED(H))) THEN
            CYCLE
          ENDIF
...
C          associated to given loops
          CALL ML5_0_2_2_HELAS_CALLS_AMPB_1(P,NHEL,H,IC)
 2000     CONTINUE
          CT_REQ_SO_DONE=.TRUE.
...
C         FeynRules, there are none of these type of counterterms.
 ...
 3000     CONTINUE
          ...
          CALL ML5_0_2_2_COEF_CONSTRUCTION_1(P,NHEL,H,IC)
 4000     CONTINUE
...
C          subroutine takes for processes with Born diagrams.
          I_SO=1
          CALL ML5_0_2_2_LOOP_CT_CALLS_1(P,NHEL,H,IC)
 5000     CONTINUE
          ...
          IF (DIRECT_ME_COMPUTATION) THEN
            DO I=1,NLOOPAMPS
              DO J=1,NLOOPAMPS
                ...
                ITEMP = ML5_0_2_2_ML5SQSOINDEX(ML5_0_2_2_ML5SOINDEX_FOR
     $_LOOP_AMP(I),ML5_0_2_2_ML5SOINDEX_FOR_LOOP_AMP(J))
...
C          if the computation is only done from the color flows
     ...
     $     .EQ.-1.OR.ML5_0_2_2_IS_HEL_SELECTED(H)))) THEN
...
C            argument of the subroutine below).
            CALL ML5_0_2_2_COMPUTE_COLOR_FLOWS(HEL_MULT)
            IF(ME_COMPUTATION_FROM_JAMP) THEN
              CALL ML5_0_2_2_COMPUTE_RES_FROM_JAMP(BUFFRES,HEL_MULT)
...
C                out this helicity
                IF (POLARIZATIONS(0,0).EQ.-1.OR.ML5_0_2_2_IS_HEL_SELECT
     $ED(H)) THEN
...
C                out this helicity
                IF (POLARIZATIONS(0,0).EQ.-1.OR.ML5_0_2_2_IS_HEL_SELECT
     $ED(H)) THEN
                  CALL ML5_0_2_2_COMPUTE_COLOR_FLOWS_DERIVED_QUANTITIES
     $(HEL_MULT)
                ENDIF
              ENDIF
            ENDIF
...
C      subroutine ML5_0_2_2_MP_COMPUTE_LOOP_COEFS to compute OpenLoop
...
C          this helicity
          IF (POLARIZATIONS(0,0).EQ.-1.OR.ML5_0_2_2_IS_HEL_SELECTED(HEL
     $PICKED)) THEN
...
C         SET THE HELICITY FILTER
          IF(.NOT.FOUNDHELFILTER) THEN
            ...
            IF(ML5_0_2_2_ISZERO(DABS(HELSAVED(1,HELPICKED))
...
C                 Be looser for helicity check, bring a factor 100
                  DUMMY=ML5_0_2_2_ISSAME(HELSAVED(1,HELPICKED)
...
C         SET THE LOOP FILTER
          IF(.NOT.FOUNDLOOPFILTER.AND.USELOOPFILTER) THEN
            DO I=1,NLOOPGROUPS
              DO J=1,NSQUAREDSO
                IF(.NOT.ML5_0_2_2_ISZERO(ABS(LOOPRES(1,J,I))
...
C         DOUBLE CHECK THE HELICITY FILTER
          IF (GOODHEL(HELPICKED).EQ.-HELOFFSET) THEN
            IF (.NOT.ML5_0_2_2_ISZERO(DABS(HELSAVED(1,HELPICKED))
 ...
 348          CONTINUE
          ...
          IF (GOODHEL(HELPICKED).LT.-HELOFFSET.AND.NTRY.NE.0) THEN
            IF(ML5_0_2_2_ISSAME(HELSAVED(1,HELPICKED),HELSAVED(1
...
C            answer from mode 1 and carry on.
     ...
     $.1).OR.((.NOT.DOING_QP_EVALS).AND.NROTATIONS_DP.GE.1)) ) THEN
          EVAL_DONE(3)=.TRUE.
          CALL ML5_0_2_2_ROTATE_PS(PS,P,1)
          IF (DOING_QP_EVALS) CALL ML5_0_2_2_MP_ROTATE_PS(MP_PS,MP_P,1)
          GOTO 200
        ENDIF
     ...
     $.2).OR.((.NOT.DOING_QP_EVALS).AND.NROTATIONS_DP.GE.2)) ) THEN
          EVAL_DONE(4)=.TRUE.
          CALL ML5_0_2_2_ROTATE_PS(PS,P,2)
          IF (DOING_QP_EVALS) CALL ML5_0_2_2_MP_ROTATE_PS(MP_PS,MP_P,2)
          GOTO 200
        ENDIF

        CALL ML5_0_2_2_ROTATE_PS(PS,P,0)
        IF (DOING_QP_EVALS) CALL ML5_0_2_2_MP_ROTATE_PS(MP_PS,MP_P,0)

C       END OF THE DEFINITIONS OF THE DIFFERENT EVALUATION METHODS

        IF(DOING_QP_EVALS.AND.LOOPLIBS_QPAVAILABLE(MLREDUCTIONLIB(I_LIB)
     $   )) THEN
          CALL ML5_0_2_2_COMPUTE_ACCURACY(QP_RES,N_QP_EVAL,ACC,ANS)
...
C          accuracies to 1.0
     ...
     $         ,.FALSE.)
              NEPS=NEPS+1
              CALL ML5_0_2_2_COMPUTE_ACCURACY(DP_RES,N_DP_EVAL,TEMP1
     $         ,TEMP)
              CALL ML5_0_2_2_COMPUTE_ACCURACY(QP_RES,N_QP_EVAL,ACC,ANS)
              IF(NEPS.LE.10) THEN
...
C              flags.
        ...
        ELSEIF(.NOT.DOING_QP_EVALS)THEN
          CALL ML5_0_2_2_COMPUTE_ACCURACY(DP_RES,N_DP_EVAL,ACC,ANS)
C         If a floating point exception was encountered during the
C          reduction,
C         the result cannot be trusted at all and we hardset all
C          accuracies to 1.0
...
C              flags.
     ...
     $             //' encountered.'
                ENDIF
                CALL ML5_0_2_2_COMPUTE_ACCURACY(DP_RES,N_DP_EVAL,ACC
     $           ,ANS)
                IF (NSQUAREDSO.NE.1) THEN
                  WRITE(*,*) 'Accuracies for each split order,'
     $             //' starting with the summed case'
...
C     Now make sure to turn off the global COLLIER cache if applicable
      CALL ML5_0_2_2_SET_COLLIER_GLOBAL_CACHE(.FALSE.)

      END

      SUBROUTINE ML5_0_2_2_CLEAR_CACHES()
C     
...
C     
      ...
      CALL ML5_0_2_2_CLEAR_COLLIER_CACHE()
      END
...
C     --=========================================--
      ...
      LOGICAL FUNCTION ML5_0_2_2_IS_HEL_SELECTED(HELID)
      IMPLICIT NONE
C     
C     CONSTANTS
C     
...
C     ----------
      ...
      ML5_0_2_2_IS_HEL_SELECTED = .TRUE.
      ...
      DO I=1,NEXTERNAL
        ...
        ENDDO
        IF(.NOT.FOUNDIT) THEN
          ML5_0_2_2_IS_HEL_SELECTED = .FALSE.
          RETURN
        ENDIF
      ENDDO
      RETURN
      ...
      LOGICAL FUNCTION ML5_0_2_2_ISZERO(TOTEST, REFERENCE_VALUE, LOOP,
     $  SOINDEX)
      IMPLICIT NONE
C     
C     CONSTANTS
...
C     ----------
      IF(ABS(REFERENCE_VALUE).EQ.0.0D0) THEN
        ML5_0_2_2_ISZERO=.FALSE.
        WRITE(*,*) '##E02 ERRROR Reference value for comparison is'
     $   //' zero.'
        STOP 1
      ELSE
        ML5_0_2_2_ISZERO=((ABS(TOTEST)/ABS(REFERENCE_VALUE))
     $   .LT.ZEROTHRES)
      ENDIF

      IF(LOOP.NE.-1) THEN
        IF((.NOT.ML5_0_2_2_ISZERO).AND.(.NOT.S(SOINDEX,LOOP))) THEN
     ...
     $     ,(ABS(TOTEST)/ABS(REFERENCE_VALUE)),' but is unstable.'
      ...
      INTEGER FUNCTION ML5_0_2_2_ISSAME(RESA,RESB,REF,USEMAX)
      IMPLICIT NONE
...
C      inconsistency, then we only allow for weight one comparisons.
      ...
      DO I=1,N_WGT_TO_TRY
        DO J=1,3
          IF (ML5_0_2_2_ISZERO(ABS(RESB(J)),REF,-1,-1)) THEN
            IF(.NOT.ML5_0_2_2_ISZERO(ABS(RESB(J))+ABS(RESA(J)),REF,-1,
     $       -1)) THEN
              GOTO 1231
            ENDIF
C           Be looser for helicity comparison, so bring a factor 100
          ELSEIF(.NOT.ML5_0_2_2_ISZERO(ABS((RESA(J)/RESB(J))
     $     -DBLE(WGT_TO_TRY(I))),1.0D0,-1,-1)) THEN
            GOTO 1231
          ENDIF
        ENDDO
        ML5_0_2_2_ISSAME = WGT_TO_TRY(I)
        RETURN
 1231   CONTINUE
      ENDDO
      END

      SUBROUTINE ML5_0_2_2_COMPUTE_ACCURACY(FULLLIST, LENGTH, ACC,
     $  ESTIMATE)
      IMPLICIT NONE
C     
C     PARAMETERS 
...
C        accuracy to 1.0d99.
      ...
      SUBROUTINE ML5_0_2_2_SET_N_EVALS(N_DP_EVALS,N_QP_EVALS)
...
C      VARIABLES FROM A GIVEN VARIABLE IN DOUBLE PRECISION
      SUBROUTINE ML5_0_2_2_SET_MP_PS(P)
      ...
      DO I=1,NEXTERNAL
        ...
        ENDDO
      ENDDO
      CALL ML5_0_2_2_MP_IMPROVE_PS_POINT_PRECISION(MP_PS)
      DO I=1,NEXTERNAL
        DO J=0,3
          MP_P(J,I)=MP_PS(J,I)
        ENDDO
...
C     --=========================================--
      ...
      SUBROUTINE ML5_0_2_2_GET_NSQSO_LOOP(NSQSO)
C     
...
C     
      ...
      SUBROUTINE ML5_0_2_2_GET_ANSWER_DIMENSION(ANS_DIM)
C     
...
C     
      ...
      INTEGER FUNCTION ML5_0_2_2_ML5SOINDEX_FOR_SQUARED_ORDERS(ORDERS)
C     
...
C     
      ...
      COMMON/ML5_0_2_2_ML5SQPLITORDERS/SQPLITORDERS
C     
C     BEGIN CODE
C     
      DO I=1,NSQSO
        ...
        ENDDO
        ML5_0_2_2_ML5SOINDEX_FOR_SQUARED_ORDERS = I
        RETURN
 1009   CONTINUE
      ENDDO

      WRITE(*,*) 'ERROR:: Stopping function ML5_0_2_2_ML5SOINDEX_FOR_SQ'
     $ //'UARED_ORDERS'
      ...
      INTEGER FUNCTION ML5_0_2_2_ML5SOINDEX_FOR_BORN_AMP(AMPID)
C     
...
C     -----------
     ...
     $   //' maximum ',NBORNAMPS
      ENDIF
      ML5_0_2_2_ML5SOINDEX_FOR_BORN_AMP = BORNAMPORDERS(AMPID)

      END

      INTEGER FUNCTION ML5_0_2_2_ML5SOINDEX_FOR_LOOP_AMP(AMPID)
C     
...
C     -----------
     ...
     $   //' maximum ',NLOOPAMPS
      ENDIF
      ML5_0_2_2_ML5SOINDEX_FOR_LOOP_AMP = LOOPAMPORDERS(AMPID)

      END


      INTEGER FUNCTION ML5_0_2_2_ML5SQSOINDEX(ORDERINDEXA, ORDERINDEXB)
C     
...
C     
      ...
      COMMON/ML5_0_2_2_ML5AMPSPLITORDERS/AMPSPLITORDERS
C     
C     FUNCTION
C     
      INTEGER ML5_0_2_2_ML5SOINDEX_FOR_SQUARED_ORDERS
C     
C     BEGIN CODE
C     
      DO I=1,NSO
     ...
     $NDEXB,I)
      ENDDO
      ML5_0_2_2_ML5SQSOINDEX=ML5_0_2_2_ML5SOINDEX_FOR_SQUARED_ORDERS(SQ
     $ORDERS)
      END

C     This is the inverse subroutine of ML5SOINDEX_FOR_SQUARED_ORDERS.
C      Not directly useful, but provided nonetheless.
      SUBROUTINE ML5_0_2_2_ML5GET_SQUARED_ORDERS_FOR_SOINDEX(SOINDEX
     $ ,ORDERS)
C     
...
C     
      ...
      WRITE(*,*) 'ERROR:: Stopping function ML5_0_2_2_ML5GET_SQUARED_OR'
     $ //'DERS_FOR_SOINDEX'
...
C      Not directly useful, but provided nonetheless.
      SUBROUTINE ML5_0_2_2_ML5GET_ORDERS_FOR_AMPSOINDEX(SOINDEX,ORDERS)
C     
...
C     
      ...
      WRITE(*,*) 'ERROR:: Stopping function ML5_0_2_2_ML5GET_ORDERS_FOR'
     $ //'_AMPSOINDEX'
...
C      completeness
      INTEGER FUNCTION ML5_0_2_2_ML5SOINDEX_FOR_AMPORDERS(ORDERS)
C     
...
C     
      DO I=1,NAMPSO
        ...
        ENDDO
        ML5_0_2_2_ML5SOINDEX_FOR_AMPORDERS = I
        RETURN
 1009   CONTINUE
      ENDDO

      WRITE(*,*) 'ERROR:: Stopping function ML5_0_2_2_ML5SOINDEX_FOR_AM'
     $ //'PORDERS'
      WRITE(*,*) 'Could not find squared orders ',(ORDERS(I),I=1,NSO)
      STOP
...
C     --=========================================--
      ...
      SUBROUTINE ML5_0_2_2_COLLIER_COMPUTE_UV_POLES(ONOFF)
C     
...
C      overwrite the choice made here
      ...
      SUBROUTINE ML5_0_2_2_COLLIER_COMPUTE_IR_POLES(ONOFF)
C     
C     This function can be called by the MadLoop user so as to chose
C      to have COLLIER
...
C      overwrite the choice made here
      ...
      SUBROUTINE ML5_0_2_2_FORCE_STABILITY_CHECK(ONOFF)
C     
...
C     
      ...
      SUBROUTINE ML5_0_2_2_SET_AUTOMATIC_CACHE_CLEARING(ONOFF)
C     
...
C     
     ...
     $   //' NRotations_DP and NRotations_QP to 0.'
      ...
      SUBROUTINE ML5_0_2_2_SET_COUPLINGORDERS_TARGET(SOTARGET)
      IMPLICIT NONE
C     
...
C     ----------
      ...
      SUBROUTINE ML5_0_2_2_SET_LEG_POLARIZATION(LEG_ID,
     $  LEG_POLARIZATION)
      IMPLICIT NONE
C     
C     ARGUMENTS
...
C     ----------
      ...
      ELSE
        ...
        ENDDO
      ENDIF
      CALL ML5_0_2_2_SET_LEG_POLARIZATIONS(LEG_ID,LEG_POLARIZATIONS)

      END

      SUBROUTINE ML5_0_2_2_SET_LEG_POLARIZATIONS(LEG_ID,
     $  LEG_POLARIZATIONS)
      IMPLICIT NONE
C     
C     CONSTANTS
...
C     leg is POLARIZATIONS(legID,1: POLARIZATIONS(legID,0)   ).
      ...
      COMMON/ML5_0_2_2_BORN_BEAM_POL/BORN_POLARIZATIONS
...
C     ----------
      ...
      SUBROUTINE ML5_0_2_2_SLOOPMATRIXHEL(P,HEL,ANS)
      IMPLICIT NONE
C     
C     CONSTANTS
C     
...
C     ----------
      USERHEL=HEL
      CALL ML5_0_2_2_SLOOPMATRIX(P,ANS)
      END

      SUBROUTINE ML5_0_2_2_SLOOPMATRIXHEL_THRES(P,HEL,ANS,PREC_ASKED
     $ ,PREC_FOUND,RET_CODE)
      IMPLICIT NONE
C     
C     CONSTANTS
...
C     
      ...
      INTEGER H,T,U
      REAL*8 ACCURACY(0:NSQUAREDSO)
      COMMON/ML5_0_2_2_ACC/ACCURACY,H,T,U
...
C     ----------
      ...
      CALL ML5_0_2_2_SLOOPMATRIXHEL(P,HEL,ANS)
...
C     Reset it to default value not to affect next runs
      ...
      SUBROUTINE ML5_0_2_2_SLOOPMATRIX_THRES(P,ANS,PREC_ASKED
     $ ,PREC_FOUND,RET_CODE)
C     
C     Inputs are:
...
C      de-allocating
C     arrays
      SUBROUTINE ML5_0_2_2_EXIT_MADLOOP()
      CALL ML5_0_2_2_DEALLOCATE_COLOR_FLOWS()
      CONTINUE
      END
