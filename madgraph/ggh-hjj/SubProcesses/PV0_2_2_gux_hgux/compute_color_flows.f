...
C      AMPL onto the color flow space.
      ...
      SUBROUTINE ML5_0_2_2_COMPUTE_COLOR_FLOWS(HEL_MULT,DO_CUMULATIVE)
      IMPLICIT NONE
      LOGICAL DO_CUMULATIVE
      INTEGER HEL_MULT
      CALL ML5_0_2_2_REINITIALIZE_JAMPS()
      CALL ML5_0_2_2_DO_COMPUTE_COLOR_FLOWS(.FALSE.)
      END SUBROUTINE

      SUBROUTINE ML5_0_2_2_COMPUTE_COLOR_FLOWS_DERIVED_QUANTITIES(HEL_M
     $ULT)
      IMPLICIT NONE
      INTEGER HEL_MULT
      CALL ML5_0_2_2_COMPUTE_AMPL2(HEL_MULT)
      CALL ML5_0_2_2_COMPUTE_JAMP2(HEL_MULT)
      CONTINUE
      END SUBROUTINE

      SUBROUTINE ML5_0_2_2_DEALLOCATE_COLOR_FLOWS()
      IMPLICIT NONE
      CALL ML5_0_2_2_DO_COMPUTE_COLOR_FLOWS(.TRUE.)
      END SUBROUTINE

      SUBROUTINE ML5_0_2_2_DO_COMPUTE_COLOR_FLOWS(CLEANUP)
      IMPLICIT NONE
C     
C     CONSTANTS 
C     
      CHARACTER*512 PROC_PREFIX
      PARAMETER ( PROC_PREFIX='ML5_0_2_2_')
...
C     Storing concatenated filenames
     ...
     $  1,     2,     2/
      DATA (CONFIG_MAP(I),I=    19,    24) /     3,     3,     4,    
     $  4,     5,     0/
      DATA (CONFIG_MAP(I),I=    25,    30) /     0,     0,     5,    
     $  0,     0,     0/
      DATA (CONFIG_MAP(I),I=    31,    36) /     6,     6,     7,    
     $  7,     1,     1/
      DATA (CONFIG_MAP(I),I=    37,    42) /     2,     2,     3,    
     $  3,     4,     4/
      DATA (CONFIG_MAP(I),I=    43,    48) /     5,     0,     0,    
     $  0,     5,     0/
      DATA (CONFIG_MAP(I),I=    49,    54) /     0,     0,     6,    
     $  6,     7,     7/

C     
C     FUNCTIONS
C     
      INTEGER ML5_0_2_2_ML5SOINDEX_FOR_BORN_AMP
      INTEGER ML5_0_2_2_ML5SOINDEX_FOR_LOOP_AMP
C     
C     GLOBAL VARIABLES
C     
      ...
      COMMON/ML5_0_2_2_AMPL/AMPL
      COMPLEX*16 JAMPL(3,NLOOPFLOWS,NLOOPAMPSO)
      COMMON/ML5_0_2_2_JAMPL/JAMPL
      COMPLEX*16 JAMPL_FOR_AMP2(3,NLOOPFLOWS,NLOOPAMPSO,NCONFIGS)
      COMMON/ML5_0_2_2_JAMPL_FOR_AMP2/JAMPL_FOR_AMP2
...
C       Initialize the LoopColorProjector
 ...
 201    CONTINUE
     ...
     $   //' from file ML5_0_2_2_LoopColorFlowCoefs.dat.'
 203    CONTINUE
        CLOSE(1)
...
C     Projection of the loop amplitudes
      DO I=1,NLOOPFLOWS
        DO J=1,SIZE(LOOPCOLORPROJECTOR(I)%AMPID)
          SOINDEX = ML5_0_2_2_ML5SOINDEX_FOR_LOOP_AMP(LOOPCOLORPROJECTO
     $R(I)%AMPID(J))
...
C     This subroutine initializes the color flow matrix.
      SUBROUTINE ML5_0_2_2_INITIALIZE_FLOW_COLORMATRIX()
      IMPLICIT NONE
C     
C     CONSTANTS 
C     
...
C     This is of course not F77 standard but widely supported by now.
     ...
     $  LOOPCOLORFLOWMATRIX
      ...
      COMMON/ML5_0_2_2_FLOW_COLOR_MATRIX/LOOPCOLORFLOWMATRIX,
     $  CMINITIALIZED
...
C       Initialize the LoopColorFlowMatrix
 ...
 701    CONTINUE
     ...
     $   //' ML5_0_2_2_BornColorFlowMatrix.dat.'
 703    CONTINUE
        CLOSE(1)
...
C     color assignation of the events.
      SUBROUTINE ML5_0_2_2_COMPUTE_JAMP2(HEL_MULT)
      IMPLICIT NONE
C     
C     CONSTANTS 
C     
...
C     FUNCTIONS
C     
      INTEGER ML5_0_2_2_ML5SQSOINDEX
C     
C     GLOBAL VARIABLES
C     
      LOGICAL CHOSEN_SO_CONFIGS(NSQUAREDSO)
      COMMON/ML5_0_2_2_CHOSEN_LOOP_SQSO/CHOSEN_SO_CONFIGS

      COMPLEX*16 JAMPL(3,NLOOPFLOWS,NLOOPAMPSO)
      COMMON/ML5_0_2_2_JAMPL/JAMPL

      REAL*8 JAMP2(NLOOPFLOWS)
      COMMON/ML5_0_2_2_JAMP2/JAMP2
...
C     Initialization
      IF(.NOT.CMINITIALIZED) THEN
        CALL ML5_0_2_2_INITIALIZE_FLOW_COLORMATRIX()
      ENDIF
...
C     Compute the Loop ME from the loop color flow amplitudes (JAMPL)
     ...
     $   *IMAG1
        DO M=1,NLOOPAMPSO
          DO N=M,NLOOPAMPSO
            ISQSO = ML5_0_2_2_ML5SQSOINDEX(M,N)
...
C     check of the color projection and computation.
      SUBROUTINE ML5_0_2_2_COMPUTE_RES_FROM_JAMP(RES,HEL_MULT)
      IMPLICIT NONE
C     
C     CONSTANTS 
C     
...
C      and is not prefixed at all
      INTEGER ML5_0_2_2_SQSOINDEX, ML5_0_2_2_SOINDEX_FOR_AMPORDERS
C     
C     GLOBAL VARIABLES
C     
      CHARACTER(512) MLPATH
      ...
      COMMON/ML5_0_2_2_AMP2_ALL/AMP2_ALL
      COMPLEX*16 JAMPL_FOR_AMP2(3,NLOOPFLOWS,NLOOPAMPSO,NCONFIGS)
      COMMON/ML5_0_2_2_JAMPL_FOR_AMP2/JAMPL_FOR_AMP2
...
C      quantity
      CALL ML5_0_2_2_REINITIALIZE_AMP2_ALL()
...
C                  statement if this is not desirable.
     ...
     $                TEMP_AMP2(K,CONFIG_I)
      ...
      SUBROUTINE ML5_0_2_2_COMPUTE_AMPL2(HEL_MULT)
C     
C     CONSTANTS 
C     
      INTEGER    NLOOPAMPS
...
C     
      ...
      DATA (CONFIG_INDEX_MAP(I),I=     1,     6) /     1,     3,     5
     $ ,     7,     9,    17/
...
C      possibly JAMPL_FOR_AMP2 if used
      SUBROUTINE ML5_0_2_2_REINITIALIZE_JAMPS()
      IMPLICIT NONE
C     
C     CONSTANTS 
C     
...
C     This subroutine resets to 0 the common arrays AMP2_ALL
      SUBROUTINE ML5_0_2_2_REINITIALIZE_AMP2_ALL()
      IMPLICIT NONE
C     
C     CONSTANTS 
C     
...
C      the stability test for example).
      SUBROUTINE ML5_0_2_2_REINITIALIZE_CUMULATIVE_ARRAYS()
      IMPLICIT NONE
C     
C     CONSTANTS 
C     
