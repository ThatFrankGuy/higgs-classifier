      SUBROUTINE ML5_0_4_5_IMPROVE_PS_POINT_PRECISION(P)
      IMPLICIT NONE
C     
C     CONSTANTS
C     
...
C     ----------
      ...
      CALL ML5_0_4_5_MP_IMPROVE_PS_POINT_PRECISION(QP_P)
      ...
      SUBROUTINE ML5_0_4_5_MP_IMPROVE_PS_POINT_PRECISION(P)
      IMPLICIT NONE
C     
C     CONSTANTS
C     
...
C     FUNCTIONS
C     
      LOGICAL ML5_0_4_5_MP_IS_PHYSICAL
C     
C     SAVED VARIABLES
C     
...
C     Check the sanity of the original PS point
      IF (.NOT.ML5_0_4_5_MP_IS_PHYSICAL(NEWP,WARNED)) THEN
        ERRCODE = 1
...
C     Now restore the precision
      IF (IMPROVEPSPOINT.EQ.1) THEN
        CALL ML5_0_4_5_MP_PSMC_IMPROVE_PS_POINT_PRECISION(NEWP,ERRCODE
     $   ,WARNED)
      ELSEIF((IMPROVEPSPOINT.EQ.2).OR.(IMPROVEPSPOINT.LE.0)) THEN
        CALL ML5_0_4_5_MP_ORIG_IMPROVE_PS_POINT_PRECISION(NEWP,ERRCODE
     $   ,WARNED)
      ENDIF
     ...
     $     //' improvement with an alternative method.'
        ...
        ENDIF
        IF (IMPROVEPSPOINT.EQ.1) THEN
          CALL ML5_0_4_5_MP_ORIG_IMPROVE_PS_POINT_PRECISION(NEWP
     $     ,ERRCODETMP,WARNED)
        ELSEIF((IMPROVEPSPOINT.EQ.2).OR.(IMPROVEPSPOINT.LE.0)) THEN
          CALL ML5_0_4_5_MP_PSMC_IMPROVE_PS_POINT_PRECISION(NEWP
     $     ,ERRCODETMP,WARNED)
        ENDIF
...
C     Report to the user or update the PS point.
 ...
 102  CONTINUE
     ...
     $   //' routine will now be supressed.'
      ...
      FUNCTION ML5_0_4_5_MP_IS_CLOSE(P,NEWP,WARNED)
      IMPLICIT NONE
C     
C     CONSTANTS
C     
...
C      ONE
     ...
     $     //' the original one',BUFFDP
      ...
      FUNCTION ML5_0_4_5_MP_IS_PHYSICAL(P,WARNED)
      IMPLICIT NONE
C     
C     CONSTANTS
C     
...
C     FOR THAT WE NEED A REFERENCE SCALE
     ...
     $       //' accurate enough, ',BUFFDPA
            ...
            WARNED=WARNED+1
          ENDIF
          ML5_0_4_5_MP_IS_PHYSICAL = .FALSE.
        ENDIF
      ENDDO
     ...
     $       //' enough, ',BUFFDPB
      ...
      SUBROUTINE ML5_0_4_5_WRITE_MOM(P)
      IMPLICIT NONE
      INTEGER    NEXTERNAL
      PARAMETER (NEXTERNAL=5)
      INTEGER    NINITIAL
      ...
      DOUBLE PRECISION ML5_0_4_5_MDOT
...
C     
     ...
     $   ,SQRT(ABS(ML5_0_4_5_MDOT(P(0,I),P(0,I))))
      ENDDO
      ...
      DOUBLE PRECISION FUNCTION ML5_0_4_5_MDOT(P1,P2)
      IMPLICIT NONE
      DOUBLE PRECISION P1(0:3),P2(0:3)
      ML5_0_4_5_MDOT=P1(0)*P2(0)-P1(1)*P2(1)-P1(2)*P2(2)-P1(3)*P2(3)
      RETURN
      END

      SUBROUTINE ML5_0_4_5_MP_WRITE_MOM(P)
      IMPLICIT NONE
      INTEGER    NEXTERNAL
      PARAMETER (NEXTERNAL=5)
      INTEGER    NINITIAL
...
C      therefore perform the cast by hand
      ...
      WRITE (*,*) '    E | px | py | pz | m '
      DO I=1,NEXTERNAL
        DOT=SQRT(ABS(ML5_0_4_5_MP_MDOT(P(0,I),P(0,I))))
        DP_DOT=DOT
     ...
     $ ,DP_PSUM(3)
      ...
      REAL*16 FUNCTION ML5_0_4_5_MP_MDOT(P1,P2)
      IMPLICIT NONE
      REAL*16 P1(0:3),P2(0:3)
      ML5_0_4_5_MP_MDOT=P1(0)*P2(0)-P1(1)*P2(1)-P1(2)*P2(2)-P1(3)*P2(3)
      RETURN
      END
...
C     PS and MP_DONE accordingly.
      ...
      SUBROUTINE ML5_0_4_5_ROTATE_PS(P_IN,P,ROTATION)
      IMPLICIT NONE
C     
C     CONSTANTS 
C     
...
C         rotation=2 => (xp=-z,yp=y,zp=x)
      ...
      SUBROUTINE ML5_0_4_5_MP_ROTATE_PS(P_IN,P,ROTATION)
      IMPLICIT NONE
C     
C     CONSTANTS 
C     
...
C     
      LOGICAL MP_DONE
      COMMON/ML5_0_4_5_MP_DONE/MP_DONE
...
C     *****************************************************************
      ...
      SUBROUTINE ML5_0_4_5_MP_ORIG_IMPROVE_PS_POINT_PRECISION(P
     $ ,ERRCODE,WARNED)
      IMPLICIT NONE
C     
C     CONSTANTS 
...
C     
     ...
     $ **2 - PT(3)**2))
      ...
      IF (.NOT.ML5_0_4_5_MP_IS_CLOSE(P,NEWP,WARNED)) THEN
        ERRCODE=999
        GOTO 100
      ENDIF
...
C     *****************************************************************
      ...
      SUBROUTINE ML5_0_4_5_MP_PSMC_IMPROVE_PS_POINT_PRECISION(P
     $ ,ERRCODE,WARNED)
      IMPLICIT NONE
C     
C     CONSTANTS 
...
C     Now find the 'x' rescaling factor
      DO I=1,NAPPROXZEROS
        CALL ML5_0_4_5_FINDX(NEWP,APPROX_ZEROS(I),XSCALE,ERROR)
        IF(ERROR.EQ.0) THEN
          GOTO 1001
        ELSE
...
C     Consistency check
 ...
 1000 CONTINUE
      ...
      SUBROUTINE ML5_0_4_5_FINDX(P,SEED,XSCALE,ERROR)
      IMPLICIT NONE
C     
C     CONSTANTS 
C     
...
C     ----------
      ...
      DO I=1,MAXITERATIONS
        CALL ML5_0_4_5_FUNCT(PVECSQ(1),XN,.FALSE.,ERR, FVAL)
        IF (ERR.NE.0) THEN
          ERROR=ERR
          GOTO 710
        ENDIF
        CALL ML5_0_4_5_FUNCT(PVECSQ(1),XN,.TRUE.,ERR, DVAL)
        IF (ERR.NE.0) THEN
          ERROR=ERR
          GOTO 710
        ENDIF
...
C     For good measure, we iterate one last time
 ...
 710  CONTINUE
      ...
      SUBROUTINE ML5_0_4_5_FUNCT(PVECSQ,X,DERIVATIVE,ERROR,RES)
      IMPLICIT NONE
C     
C     CONSTANTS 
C     
