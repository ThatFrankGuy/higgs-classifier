...
C     
      ...
      COMMON/ML5_0_1_2_CHOSEN_LOOP_SQSO/CHOSEN_LOOP_SO_CONFIGS
C     
C     CHECK BUFFERING
C     
      DO I=1, BUFFER_SIZE
...
C     BEGIN CODE
C     
      CALL ML5_0_1_2_FORCE_STABILITY_CHECK(.TRUE.)
      CALL ML5_0_1_2_GET_ANSWER_DIMENSION(MATELEM_ARRAY_DIM)
      ALLOCATE(MATELEM(0:3,0:MATELEM_ARRAY_DIM))
      CALL ML5_0_1_2_GET_NSQSO_LOOP(NSQUAREDSO_LOOP)
...
C     
      IF (NHEL.EQ.0) THEN
        CALL ML5_0_1_2_SLOOPMATRIX_THRES(P,MATELEM,-1.0D0, PREC_FOUND,
     $    RETURNCODE)
      ELSE
        CALL ML5_0_1_2_SLOOPMATRIXHEL_THRES(P,NHEL, MATELEM,-1.0D0,
